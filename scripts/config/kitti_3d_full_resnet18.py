...
def Config():
    ...
    conf.model = 'resnet18_3d_dilate_decomp_alpha_un_kalman'
    ...
    conf.freeze_blacklist = ['pose', 'conf']
    
    conf.pretrained = 'output/kitti_3d_uncertainty_resnet18/weights/model_50000_pkl'
    conf.copy_stats = True
    conf.progressive = True
    
    conf.has_vel = True
    
    conf.train_pose = True
    conf.train_det_t = False
    conf.train_det_c = False
    ...
    conf.datasets_train = [{'name': 'kitti_split1', 'anno_fmt': 'kitti_tracking', 'im_ext': '.png', 'scale': 1}]
    ...
    conf.pose_lambda_t = 1
    conf.pose_lambda_r = 40
    
    conf.bbox_un_lambda = 0
    conf.bbox_un_lambda = 0
    conf.bbox_un_dynamic = True
